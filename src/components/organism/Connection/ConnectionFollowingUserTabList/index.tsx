...
import { useGetUserFollowing, useGetUserMutuals } from "@/services/connection";
import MembersUserCard from "@/components/molecules/MembersUserCard";
import SearchInput from "@/components/atoms/SearchInput";

const ConnectionFollowingUserTabList: React.FC<{}> = ({}) => {
  ...
  const userProfiles = useMemo(
    () =>
      userFollowingData?.pages
        ?.flatMap(({ users }) => users)
        .filter(({ _id }) => _id !== userProfileData?.users_id) ?? [],
    [userFollowingData, userProfileData?.users_id],
  );
...
export default ConnectionFollowingUserTabList;
