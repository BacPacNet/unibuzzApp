...
const handleFollowClick = async (id: string) => {
    ...
    <ReusableButton onPress={() => handleNavigate(item._id)} variant="border"  buttonText="View Profile"  height="medium" size={100} />

    ) : (

    <ReusableButton variant="primary"   buttonContent={<View className="flex-row items-center justify-center gap-1">
        <Text className="text-white font-bold text-2xs">Follow</Text>
        <UserPlus height={16} width={16} color={"white"} fill={"white"} />
      </View>} onPress={() => handleFollowClick(item._id)} height="medium" size={90} disabled={isProcessing}   isLoading={isProcessing} />
    );
  ...
  <View   className="flex justify-end w-max">
  {renderCTA}
  </View>
