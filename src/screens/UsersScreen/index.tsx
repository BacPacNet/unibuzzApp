...
import { useGetUserFollowers, useGetUserFollowing } from "@/services/connection";
...
const UsersScreen = ({ route }: any) => {
    ...
    const  userProfileData  = getUserProfileStore()
      ...
      } = useGetUserFollowing('',  10, listType.following.toString() == type,userId)
      ...
      } = useGetUserFollowers('',  10, listType.following.toString() == type,userId)
    ...
    const userFollow = useMemo(() => {
        ...
        return (
          userFollowData?.pages.flatMap((page) =>
            page.users
              .filter((user) => (userProfileData?.users_id === userId ? user._id !== userProfileData?.users_id : true))
              .map((user) => ({
                ...user,
    ...
    const handleBack = () => {
      ...
      const isLoading = listType.following.toString() == type ? isFollowingLoading : isFollowersLoading
      const userList = listType.following.toString() == type ? userFollow : userFollowers
      const isFetchingNextPage =listType.following.toString() == type ? isFetchingFollowingNextPage : isFetchingFollowersNextPage
