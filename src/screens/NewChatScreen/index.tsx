...
export default function NewChatScreen() {
  ...
  const { mutateAsync: createGroupChat, isPending: groupChatPending } = useCreateGroupChat()
  ...
  const handleIndividualUserClick = async () => {
    if(selectedType === "group"){
        ...
        if (imageToUpload) {
          ...
          ImageData = { groupLogo: { imageUrl: res.data[0]?.imageUrl, publicId: res.data[0]?.publicId } }
       ...
       const createChatResponse: any = await createGroupChat(dataTopush)
        ...
        const createChatResponse: any = await mutateCreateUserChat({ userId: selectedUsers[0]._id })
        navigate.navigate("Messages", { selectedUserId: createChatResponse._id });
    }
  ...
  options={[
    {
      ...
      content: (
        ...
        label={"Select User"}
        text={"Select User"}
        onPress={() => {userActionSheetRef.current?.show()}}
        toShowCross={false}
